export class CPUAI {
    ...
    applyCard(cardName, canvas, game) {
        const that = this;
        const card = this.cards.getSingleCard(cardName);
        const backOfCardObject = this.cards.getBackOfCardObject(cardName);
        const cardObject = this.cards.getCardObject(cardName);
    ...
    discardCard(cardName, canvas, game) {
        ...
        const card = this.cards.getSingleCard(cardName);
        const backOfCardObject = this.cards.getBackOfCardObject(cardName);
        const backOfCardObjectTop = backOfCardObject.getTop();
        if (game.isOn() && game.getPlayerTurn(that.cpu)) {
            if (game.cardAvailable(cardName, that.cpu)) {
                game.playerMoved(that.cpu);
    ...
    move(canvas, game, CPUcards) {
        ...
        if (!that.cardsCanBeUsed(CPUcards)) {
            ...
            const availableCards = that.getAllCardsThatCanBeUsed(CPUcards);
            const mostResCardName = that.getMostResourcefulCard(availableCards);
