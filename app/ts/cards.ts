import {ArcomageCardsContainer} from "./cards_container";
import {Player} from "./players";

export class ArcomageCards extends ArcomageCardsContainer {
    ...
    public getCardNameByDesc(description: string): string {
        for (let i = 0, length = this.names.length; i < length; i++) {
            const cardDesc = this.getSingleCard(this.names[i]).description;
    ...
    public cardCanBeUsed(cardName: string, player: Player): boolean {
        const card = this.getSingleCard(cardName);
        const resourceName: string = Object.keys(card.resource)[0];
        const resourceValue: number = card.resource[resourceName];
