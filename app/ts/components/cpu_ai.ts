...
export class CPUAI {
    ...
    public applyCard(cardName: string, canvas: Canvas, game: Arcomage): void {
        ...
        const backOfCardObject: IGroup = this.cards.getBackOfCardObject(cardName);
    ...
    public discardCard(cardName: string, canvas: Canvas, game: Arcomage): void {
        ...
        const card: any = this.cards.getSingleCard(cardName);
        const backOfCardObject: IGroup = this.cards.getBackOfCardObject(cardName);
        const backOfCardObjectTop: number = backOfCardObject.getTop();
    ...
    public move(canvas: Canvas, game: Arcomage, CPUcards: any[]): void {
        const allCards: any = this.cards.cards;
        const that = this;
        ...
        if (!that.cardsCanBeUsed(CPUcards)) {
            ...
            const availableCards: any = that.getAllCardsThatCanBeUsed(CPUcards);
            const mostResCardName: string = that.getMostResourcefulCard(availableCards);
